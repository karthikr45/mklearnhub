...
# App task role: read/write the media bucket.
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "ENABLE_QUEUES", value = "true" },
        { name = "ENABLE_SEARCH", value = "true" },
        ...
        { name = "DATABASE_URL", value = "postgresql://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.endpoint}/learnhub_db?sslmode=require" }
      ]
      logConfiguration = {
        logDriver = "awslogs"
        options = {
...
# ── Worker task definition + service (BullMQ consumers, no ALB) ──────────────
